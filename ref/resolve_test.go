...
func TestImageResolve(t *testing.T) {
	...
	rr, err = MustNewContainerImageReference("mvanholsteijn/paas-monitor:latest").ResolveDigest()
	if err != nil {
		t.Fatal(err)
	}
	if digest.String() == rr.Digest {
...
func TestImageResolves(t *testing.T) {
	references := ContainerImageReferences{*MustNewContainerImageReference(`mvanholsteijn/paas-monitor:latest`),
