...
func TestUpdateReferenceSimple(t *testing.T) {
	...
	expect := []byte(`eu.gcr.io/binxio/paas-monitor:v0.3.2`)
	ref := MustNewContainerImageReference(`eu.gcr.io/binxio/paas-monitor:v0.3.2`)
...
not be changed`)
	expect := []byte(`this is one eu.gcr.io/binxio/paas-monitor:v1.0.0 reference
and this is another mvanholsteijn/paas-monitor:v0.2.0-beta
and this is just a directory name mvanholsteijn/paas-monitor, which should
not be changed`)
	references := []ContainerImageReference{*MustNewContainerImageReference(`eu.gcr.io/binxio/paas-monitor:v1.0.0`),
...
resource "google_cloud_run_service" "app" {
  ...
  template {
    spec {
      ...
      containers {
        image = "eu.gcr.io/binxio/paas-monitor:v0.3.1"
      }
    }
  }
  timeouts {
...
resource "google_cloud_run_service" "app" {
  ...
  template {
    spec {
      service_account_name = google_service_account.packer-reaper.email
      containers {
        image = "eu.gcr.io/binxio/paas-monitor:v0.3.2"
      }
    }
  }
  timeouts {
...
`)
	ref, _ := NewContainerImageReference(`eu.gcr.io/binxio/paas-monitor:v0.3.2`)
